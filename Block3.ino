...
const int win_matrix[3][3] = {
  // Column = Player1 chose rock, paper or scissors
  // Row = Player2 chose rock paper or scissors
  { 0,                  1,                  -1 },
  { -1,                 0,                   1 }, 
  { 1,                  -1,                  0 }
};
...
void loop()
{
  ...
  {
    lcd.print("pick your poison guzzo1")

    // finalChoice button stores answer to variable player1Final 

    while (!digitalRead(finalChoice)) {} // Wait for button press
    player1Final = (digitalRead(rockButton) == HIGH) ? 0 : (digitalRead(paperButton) == HIGH) ? 1 : 2; // Map button press to choice (0 for rock, 1 for paper, 2 for scissors)
  ...
  {
    lcd.print("pick your poison guzzo2")
    
    // finalChoice button stores answer to variable player2Final  

    while (!digitalRead(finalChoice)) {} // Wait for button press
    player2Final = (digitalRead(rockButton) == HIGH) ? 0 : (digitalRead(paperButton) == HIGH) ? 1 : 2; // Map button press to choice (0 for rock, 1 for paper, 2 for scissors)
    state == 2;
  }
